...
export const Navbar: React.FC<NavbarProps> = ({ onOpenWhatsApp }) => {
  ...
  const navLinks = [
    { name: 'About Flyspurs', path: '/about' },
        ...
        <nav className="hidden xl:flex items-center gap-1 xl:gap-1.5">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              className={`px-2.5 py-1.5 text-xs xl:text-sm font-semibold rounded-lg transition-all whitespace-nowrap ${
          ...
          <nav className="flex flex-col space-y-1 divide-y divide-[#F3F4F6]">
            {navLinks.map((link) => (
              <Link
                ...
                className={`py-3 px-2 text-sm font-semibold rounded-lg transition-colors flex items-center justify-between ${
                  location.pathname === link.path
                    ? 'text-[#0A2D68] bg-[#EEF5FF] font-bold'
                    : 'text-[#374151] hover:text-[#0A2D68] hover:bg-[#EEF5FF]'
                }`}
              >
                <span>{link.name}</span>
